...
def check_positions(token,chat_id,cfg):
    ...
    for pos in positions:
        ...
        if age_days>=expiry_days:
            pos["status"]="expired"; pos["close_ts"]=now.isoformat(timespec="seconds"); pos["close_price"]=price; pos["expiry_days"]=age_days; alerts.append(_fmt_msg("expired",coin,pos,price)); store.position_event("expired",coin,entry,price=price); continue
        closed=False
        if price<=pos["sl"]:
            pos["status"]="closed_sl"; closed=True; alerts.append(_fmt_msg("sl",coin,pos,price)); store.position_event("sl",coin,entry,price=price)
            ...
            pos["tp3_hit"]=True; pos["status"]="closed_tp3"; closed=True; alerts.append(_fmt_msg("tp3",coin,pos,price)); store.position_event("tp3",coin,entry,price=price,outcome="tp3")
        if closed:
