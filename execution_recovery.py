...
class ExecutionSafety:
    ...
    def from_env(cls) -> "ExecutionSafety":
        return cls(max_notional_usdt=float(os.getenv("EXECUTION_MAX_NOTIONAL_USDT", "300")), max_active_intents=int(os.getenv("EXECUTION_MAX_ACTIVE_INTENTS", "2")), max_daily_notional_usdt=float(os.getenv("EXECUTION_MAX_DAILY_NOTIONAL_USDT", "1000")), kill_switch=os.getenv("EXECUTION_KILL_SWITCH", "false").lower() in {"1","true","yes","on"})
    def validate_notional(self, notional: float) -> None:
        if self.kill_switch: raise ValueError("execution kill switch is active")
        if notional <= 0 or notional > self.max_notional_usdt: raise ValueError("notional exceeds execution safety limit")

def can_transition(current: str, target: str) -> bool: return target in TRANSITIONS.get(current, set())
def transition(record: dict[str, Any], target: str) -> dict[str, Any]:
    current = str(record.get("status", "PENDING_CONFIRMATION"))
    if current in TERMINAL: raise ValueError(f"terminal intent cannot transition: {current}")
    if not can_transition(current, target): raise ValueError(f"invalid transition: {current} -> {target}")
    record["status"] = target; return record

def recover_active_intents(records: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    latest: dict[str, dict[str, Any]] = {}
    for record in records:
        intent_id = str(record.get("intent_id", ""))
        if intent_id: latest[intent_id] = record
...
def append_record(path: str | Path, record: dict[str, Any]) -> None:
    ...
    with p.open("a", encoding="utf-8") as fh: fh.write(json.dumps(record, separators=(",",":"), sort_keys=True)+"\n")
