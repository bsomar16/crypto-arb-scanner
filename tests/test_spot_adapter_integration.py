...
class SpotAdapterIntegrationHarnessTests(unittest.TestCase):
    ...
    def test_authenticated_spot_order_path_for_http_json_adapters(self):
        ...
        for adapter, target, response in cases:
            with self.subTest(adapter=adapter.name), patch(target, return_value=response) as request:
                ...
                payload = request.call_args.kwargs.get("params") or request.call_args.kwargs.get("body") or {}
    ...
    def test_invalid_order_type_is_rejected_before_transport(self):
        for module in ("adapters.binance", "adapters.bybit", "adapters.okx",
                       "adapters.bitget_spot", "adapters.mexc_spot"):
            with self.subTest(module=module):
                target = module + (".request_json" if module in ("adapters.binance", "adapters.bybit", "adapters.okx") else ".urlopen")
                with patch(target) as transport:
                    if module == "adapters.binance":
                        adapter = BinanceSpotAdapter("k", "s")
                    elif module == "adapters.bybit":
                        adapter = BybitSpotAdapter("k", "s")
                    elif module == "adapters.okx":
                        adapter = OKXSpotAdapter("k", "s", "p")
                    elif module == "adapters.bitget_spot":
                        adapter = BitgetSpotAdapter("k", "s", "p")
                    else:
                        adapter = MexcSpotAdapter("k", "s")
