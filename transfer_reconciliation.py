...
def _snapshot(provider: str, direction: str, row: Dict[str, Any]) -> TransferSnapshot:
    ...
    return TransferSnapshot(provider, direction, _norm_status(status), str(pid), str(tx), float(amount or 0),
...
def _query(adapter: Any, direction: str, *, transfer_id: str, asset: str, network: str, created_ms: int = 0) -> TransferSnapshot:
    provider = str(adapter.name).lower()
    ...
    if provider == "binance":
        ...
        if direction == "deposit":
            rows = [r for r in rows if str(r.get("txId") or r.get("id") or "") == transfer_id or (created_ms and int(r.get("insertTime", 0) or 0) >= created_ms)]
    elif provider == "bybit":
        ...
        data = adapter._private("GET", path, params=params)
        rows = _rows(data)
...
def reconcile_transfer(source_adapter: Any, destination_adapter: Any, *, transfer_id: str, asset: str,
    ...
    destination_key = source.tx_hash or transfer_id
    destination = _query(destination_adapter, "deposit", transfer_id=destination_key, asset=asset, network=network, created_ms=created_ms)
